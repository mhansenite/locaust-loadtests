...
class ProjectPhaseMilestoneLoadTest(AuthenticatedUser):
    ...
    Tests creating projects (optionally), phases and milestones, tasks, and various
    task update operations using the APIs discovered from HAR file analysis.
    ...
    - create_task (6): Task creation (1-10 tasks per call) - increased for better buildup
    - view_project_plan (5): Project plan viewing activity - moderate
    - create_milestone (5): Milestone creation - increased for better buildup
    - create_test_phase (4): Phase creation - increased for better buildup
    ...
    def _create_single_task(self, milestone_id, milestone_name, task_suffix=""):
        ...
        if task_info:
            # Store the created task for tracking (phase_id, project_id now come from creation)
    ...
    @task(5)  # Moderate weight for viewing activity
    def view_project_plan(self):
        ...
        if not self.test_project_id:
            print(f"⚠️ No project ID available for view_project_plan (skipping task)")
            return
    ...
    @task(5)  # Increased weight - need to create milestones faster for task buildup
    def create_milestone(self):
        """Test creating a milestone in the current phase"""
        # Ensure we have a project to work with (project should be created in on_start)
        if not self.test_project_id:
            print(f"⚠️ No project ID available for create_milestone (skipping task)")
            return
    ...
    @task(6)  # Increased weight - need to create tasks faster for status update testing
    def create_task(self):
        """Test creating 1-10 random tasks under a milestone"""
        # Ensure we have a project to work with (project should be created in on_start)
        if not self.test_project_id:
            print(f"⚠️ No project ID available for create_task (skipping task)")
            return
        ...
        # Add milestone_name to each task and track them (phase_id, project_id now come from creation)
    ...
    @task(8)  # High frequency - time tracking is very common
    def update_task_time_tracking(self):
        ...
        if not self.created_tasks:
            print(f"No tasks available for time tracking, creating tasks first...")
            self.create_task()
            
            # Check if we have tasks after creation attempt
            if not self.created_tasks:
                print(f"⚠️ Task creation failed, cannot add time tracking")
                return
        
        # Randomly select 1 task to add time tracking
        task_info = random.choice(self.created_tasks)
        ...
        print(f"Adding time tracking to task '{task_name}' (ID: {task_id[:8]}): {hours_worked:.1f}h")
        ...
        print(f"  ✅ Time tracking simulated: {hours_worked:.1f}h - '{time_comment}'")
        return True

    @task(8)  # High frequency - date adjustments are very common
    def update_task_dates(self):
        """Test updating task start and due dates using project plan API"""
        # Create tasks if none exist yet
        if not self.created_tasks:
            print(f"No tasks available for date updates, creating tasks first...")
            self.create_task()
            
            # Check if we have tasks after creation attempt
            if not self.created_tasks:
                print(f"⚠️ Task creation failed, cannot update task dates")
                return
        
        # Randomly select 1-2 tasks to update dates
        num_updates = min(random.randint(1, 2), len(self.created_tasks))
        tasks_to_update = random.sample(self.created_tasks, num_updates)
        
        print(f"Updating dates for {num_updates} task(s)...")
        ...
        for task_info in tasks_to_update:
            ...
            print(f"  ✅ Date update simulated for '{task_name}': Due {due_date.strftime('%Y-%m-%d')}")
        
        return True

    @task(7)  # High frequency - effort estimation adjustments are common
    def update_task_estimated_hours(self):
        """Test updating task estimated hours using project plan API"""
        # Create tasks if none exist yet
        if not self.created_tasks:
            print(f"No tasks available for hours estimation, creating tasks first...")
            self.create_task()
            
            # Check if we have tasks after creation attempt
            if not self.created_tasks:
                print(f"⚠️ Task creation failed, cannot update estimated hours")
                return
        
        # Randomly select 1-2 tasks to update estimates
        num_updates = min(random.randint(1, 2), len(self.created_tasks))
        tasks_to_update = random.sample(self.created_tasks, num_updates)
        
        print(f"Updating estimated hours for {num_updates} task(s)...")
        ...
        for task_info in tasks_to_update:
            ...
            # TODO: Implement actual API call to project plan endpoint
            print(f"  ✅ Hours estimate updated for '{task_name}': {estimated_hours}h")
        
        return True

    @task(7)  # High frequency - assignment changes are common
    def update_task_assignments(self):
        """Test updating task assignments using project plan API"""
        # Create tasks if none exist yet
        if not self.created_tasks:
            print(f"No tasks available for assignment updates, creating tasks first...")
            self.create_task()
            
            # Check if we have tasks after creation attempt
            if not self.created_tasks:
                print(f"⚠️ Task creation failed, cannot update task assignments")
                return
        
        # Randomly select 1 task to reassign
        task_info = random.choice(self.created_tasks)
        task_id = task_info['id']
        task_name = task_info['name']
        ...
        print(f"  ✅ Assignment updated for '{task_name}': Assigned to {new_assignee}")
        
        return True

    @task(8)  # High frequency - status updates are very common
    def update_task_status(self):
        """Test updating task status using project plan API with dynamic status extraction"""
        # Ensure we have a project to work with (project should be created in on_start)
        if not self.test_project_id:
            print(f"⚠️ No project ID available for update_task_status (skipping task)")
            return
        
        # Create tasks if none exist yet
        if not self.created_tasks:
            print(f"No tasks available for status update, creating tasks first...")
            self.create_task()
            
            # Check if we have tasks after creation attempt
            if not self.created_tasks:
                print(f"⚠️ Task creation failed, cannot update task status")
                return
        
        # Randomly select 1 task to update status
        task_info = random.choice(self.created_tasks)
        task_id = task_info['id']
        task_name = task_info['name']
        task_phase_id = task_info.get('phase_id', self.test_phase_id)  # Use task's phase or fallback
        ...
        print(f"Getting available statuses for task status update...")
        ...
        if not available_statuses:
            print(f"⚠️ Could not fetch statuses from API, using fallback statuses")
        ...
        print(f"Selected status '{new_status}' from {len(available_status_names)} available options")
        ...
        if success:
            print(f"  ✅ Status updated for '{task_name}': {new_status}")
        else:
            print(f"  ❌ Failed to update status for '{task_name}' to '{new_status}'")
        
        return success


      
    @task(2)  # Lower weight - phase creation happens less frequently
    def create_test_phase(self):
        """Test creating a new phase in the project using the common create_phase function"""
        # Ensure we have a project to work with (project should be created in on_start)
        if not self.test_project_id:
            print(f"⚠️ No project ID available for create_test_phase (skipping task)")
            return
    ...
    def on_start(self):
        ...
        # Create project ONCE per user if needed
        if self.CREATE_NEW_PROJECT and not self.test_project_id:
            print(f"Creating project for this user (CREATE_NEW_PROJECT=true)...")
        ...
        if self.CREATE_NEW_PROJECT:
            if self.test_project_id:
                print(f"Project ID: Created NEW project: {self.test_project_id}")
            else:
                print(f"Project ID: Failed to create, using fallback: {self.PROJECT_ID}")
