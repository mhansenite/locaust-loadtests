...
LOGIN_EMAIL = "mhansen+thundercats@guidecx.com"
LOGIN_PASSWORD = "DKD8x$Ad^3KPRU"


# Default Load Testing Parameters
DEFAULT_USERS = 10          # Number of concurrent users to simulate
DEFAULT_SPAWN_RATE = 2      # Users to spawn per second
DEFAULT_RUN_TIME = "60s"    # How long to run the test (60s, 5m, 1h, etc.)

# Debug Configuration
DEBUG = False               # Set to False to disable debug prints (recommended for headless mode)

# READY-TO-RUN LOCUST COMMANDS (copy/paste these):
# locust -f loginnew.py --headless --users=10 --spawn-rate=2 --run-time=60s    # Default load test
# locust -f loginnew.py --headless --users=25 --spawn-rate=3 --run-time=120s   # Medium load test  
# locust -f loginnew.py --headless --users=50 --spawn-rate=5 --run-time=300s   # Heavy load test
# locust -f loginnew.py                                                        # Interactive web UI

# Performance Configuration
WAIT_TIME_BETWEEN_TASKS = 1  # Seconds to wait between API calls per user
